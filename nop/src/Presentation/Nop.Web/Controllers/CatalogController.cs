    ...
    public partial class CatalogController : BaseNopController
    {
            ...
            foreach (var product in products)
            {
                ...
                if (preparePictureModel)
                {
                    ...
                    model.DefaultPictureModel = _cacheManager.Get(defaultProductPictureCacheKey, () =>
                    {
                        ...
                        return preparePicModel(products.Where(m => m.Id == product.Id).FirstOrDefault(), pictureModel);
                    });
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                defaultPictureModel = preparePicModel(_productService.GetProductById(product.Id), defaultPictureModel);
            ...
            foreach (var attribute in productVariantAttributes)
            {
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var pvaValue in pvaValues.Where(m => m.DisplayOrder >= 0))
                    {
            ...
            var customerRolesIds = _workContext.CurrentCustomer.CustomerRoles
                .Where(cr => cr.Active).Select(cr => cr.Id).ToList();
            string cacheKey = string.Format(ModelCacheEventConsumer.PRODUCT_MANUFACTURERS_MODEL_KEY, product.Id, _workContext.WorkingLanguage.Id, string.Join(",", customerRolesIds), _storeContext.CurrentStore.Id);
            var cacheModel = _cacheManager.Get(cacheKey, () =>
            {
                var modelm = _manufacturerService.GetProductManufacturersByProductId(product.Id)
                    .Select(x =>
                    {
            ...
            if (cacheModel != null)
            {
                model.ManufactureName = cacheModel.Name;
                model.ManufactureSeName = cacheModel.SeName;
            }
        ...
        [DonutOutputCache(CacheProfile = "Category")]
        ...
        [NopHttpsRequirement(SslRequirement.No)]
        [DonutOutputCache(CacheProfile = "Manufacturer")]
        ...
        [NopHttpsRequirement(SslRequirement.No)]
        [DonutOutputCache(CacheProfile="ProductDetail")]
        ...
        [HttpPost]
        public JsonResult GetStoreProductStocking(int productId)
        {
            var cfStores = _productService.GetStoreProductStocking(productId)
                .Select(m => m.ToModel()).Select(a => new {a.Name,a.Address}).ToList();

            var result = Json(cfStores);
            return result;
        }

        [HttpPost]
        public JsonResult GetInventoryQuantity(int productId)
        {
            var quantity = _productService.GetInventoryQuantity(productId);
            var result = Json(quantity);
            return result;
        }
        #endregion
        ...
        private PictureModel preparePicModel(Product productVariant, PictureModel model = null)
        {
            if (model == null) model = new PictureModel();
            int discountPercent = 0;
            if (productVariant != null)
            {
                decimal maxPrice = productVariant.Price;
                if (maxPrice != 0)
                {
                    var price = _priceCalculationService.GetFinalPrice(productVariant, _workContext.CurrentCustomer, true);
                    discountPercent = (int)Math.Round(((maxPrice - price) / maxPrice) * 100);
                    if (discountPercent > 0)
                    {
                        model.IsDiscount = true;
                        model.DiscountPercent = string.Format("{0}%", discountPercent);
                    }
                }
