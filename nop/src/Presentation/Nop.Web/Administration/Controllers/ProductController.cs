...
namespace Nop.Admin.Controllers
{
    ...
    public partial class ProductController : BaseNopController
    {
        ...
        public ActionResult List()
        {
            ...
            ViewBag.AllowDeleteProducts = _permissionService.Authorize(StandardPermissionProvider.DeleteProducts);
            ViewBag.AllowPublishProducts = _permissionService.Authorize(StandardPermissionProvider.PublishProducts);
            ViewBag.CanEditProduct = _permissionService.Authorize(StandardPermissionProvider.Aero87EditProduct);
            ...
            foreach (var m in EngineContext.Current.Resolve<ICommonService>().GetAllStore())
                model.Stores.Add(new SelectListItem() { Text = m.Name, Value = m.Id.ToString(), Selected = m.Id == -1 });
        ...
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts) || 
                !_permissionService.Authorize(StandardPermissionProvider.Aero87EditProduct))
                return AccessDeniedView();

            ViewBag.AllowDeleteProducts = _permissionService.Authorize(StandardPermissionProvider.DeleteProducts);
            ViewBag.CanEditPrice = _permissionService.Authorize(StandardPermissionProvider.Aero87EditPrice);
        ...
        {
            if (!(_permissionService.Authorize(StandardPermissionProvider.ManageProducts) 
                && _permissionService.Authorize(StandardPermissionProvider.DeleteProducts)) )
                return AccessDeniedView();
        ...
        public ActionResult DeleteSelected(string selectedIds)
        {
                ...
                && _permissionService.Authorize(StandardPermissionProvider.DeleteProducts)))
            {
                ...
                var url = urlHelper.Action("AccessDenied", "Security", new { pageUrl = this.Request.RawUrl });
                return Json(new { Status = "Redirect", Url = url }); 
            }
            ...
            if (selectedIds != null)
            {
                ...
                try
                {
                    ...
                    return Json(new { Status = "Success", Message = _localizationService.GetResource("Admin.Catalog.Products.Deleted") });                    
                }
                catch(Exception ex)
                {
        ...
        {
            if (!(_permissionService.Authorize(StandardPermissionProvider.ManageProducts)
                && _permissionService.Authorize(StandardPermissionProvider.PublishProducts)))
            {
                //return AccessDeniedView();
                var urlHelper = new UrlHelper(Request.RequestContext);
                var url = urlHelper.Action("AccessDenied", "Security", new { pageUrl = this.Request.RawUrl });
            ...
            if (selectedIds != null)
            {
                ...
                try
                {
                    ...
                    return Json(new { Status = "Success", Message = _localizationService.GetResource("Admin.Catalog.Products.Updated") });
                }
                catch (Exception ex)
                {
                    return Json(new { Status = "Error", Message = ex.Message });
        ...
        [HttpPost]
        public ActionResult Aero87ImportProductsFromXlsx(bool? IsEditProduct, bool? IsEditInvoice, bool? IsPublished, int importedStoreId)
        {
            ...
            try
            {
                if (!IsEditInvoice.GetValueOrDefault() && !IsEditProduct.GetValueOrDefault())
                {
                    //throw new InvalidDataException();
                    throw new ArgumentNullException("EditProduct and EditInvoice");
                }

                var file = Request.Files["importexcelfile"];
                if (file != null && file.ContentLength > 0)
                {
                    _importManager.Aero87ImportProductsFromXlsx(file.InputStream, IsEditProduct.GetValueOrDefault(), IsEditInvoice.GetValueOrDefault(), IsPublished.GetValueOrDefault(), importedStoreId);
                }
                else
                {
                    ErrorNotification(_localizationService.GetResource("Admin.Common.UploadFile"));
