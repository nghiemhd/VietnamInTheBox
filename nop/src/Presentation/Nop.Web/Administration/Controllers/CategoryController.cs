...
namespace Nop.Admin.Controllers
{
    ...
    public partial class CategoryController : BaseNopController
    {
        ...
        [HttpPost]
        public ActionResult ProductDeleteIds(int[] ids)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();

            if (ids == null) return Json(null);
        ...
        [HttpPost]
        public ActionResult RemoveAllProducts(int categoryId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();

            if (categoryId == 0) return Json(null);

            _categoryService.DeleteAllProductsByCategoryId(categoryId);
        ...
        public ActionResult ProductAddPopupList(GridCommand command, CategoryModel.AddCategoryProductModel model)
        {
            ...
            var products = _productService.SearchProducts(
                ...
                showHidden: true,
                publishStatus: model.SearchProductStatus
                );
            //gridModel.Data = products.Select(x => x.ToModel());

            var productList = products.Select(x => x.ToModel()).ToList();
            foreach (var product in productList)
            {
                var defaultProductPicture = _pictureService.GetPicturesByProductId(product.Id, 1).FirstOrDefault();
                var pictureThumbnailUrl = _pictureService.GetPictureUrl(defaultProductPicture, 75, true);
                product.PictureThumbnailUrl = pictureThumbnailUrl;
            }
            gridModel.Data = productList;
