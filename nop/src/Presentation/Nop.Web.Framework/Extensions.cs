...
namespace Nop.Web.Framework
{
    public static class Extensions
    {
        ...
        public static SelectList ToSelectListWithDisplayText<TEnum>(this TEnum enumObj, bool markCurrentAsSelected = true) 
            where TEnum : struct 
        {
            if (!typeof(TEnum).IsEnum) throw new ArgumentException("An Enumeration type is required.", "enumObj");

            var values = from TEnum enumValue in Enum.GetValues(typeof(TEnum))
                         select new { ID = Convert.ToInt32(enumValue), Name = enumValue.GetEnumDisplayText() };
            object selectedValue = null;
            if (markCurrentAsSelected)
                selectedValue = Convert.ToInt32(enumObj);
            return new SelectList(values, "ID", "Name", selectedValue);
        ...
        public static string GetEnumDisplayText<TEnum>(this TEnum enumValue) where TEnum : struct
        {
            var display = enumValue.ToString();
            var type = enumValue.GetType();
            var memInfo = type.GetMember(enumValue.ToString());
            var attribute = memInfo[0].GetCustomAttributes(typeof(DisplayAttribute), false).FirstOrDefault();
            if (attribute != null && ((DisplayAttribute)attribute).Name != string.Empty)
            {
                display = ((DisplayAttribute)attribute).Name;
            }
