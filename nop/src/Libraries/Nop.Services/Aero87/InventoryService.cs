...
namespace Nop.Services.Aero87
{
    public class InventoryService : IInventoryService
    {
        ...
        public void IVMTranITComplete(int transferId, int typeId, string user)
        {
            ...
            _dbContext.ExecuteSqlCommand(@"exec usp_IVMTranByIT_Complete @transferId,@typeId,@user", false, 10000,
                ptransferId,ptypeId,puser
                );
        }
            ...
            )
        {
            ...
            var rs = _dbContext.SqlQuery<WarehouseCountResult>(@"exec usp_WarehouseCount  @storeId, @manufacturerId, @categoryIds, @orderByAge, @productCode, @filterByQty,@toEndOfBeforeDate, @pageIndex, @pageSize, @count output, @sumQty output, @sumCostAmount output",
        ...
        public IList<WarehouseCountLineResult> GetWarehouseCountDetail(int productId, int? storeId, DateTime toEndDate)
        {
            ...
            var rs = _dbContext.SqlQuery<WarehouseCountLineResult>(@"exec usp_WarehouseCountDetail @storeId,@productId,@toEndOfBeforeDate", pstoreId, pproductId, ptoEndOfBeforeDate).ToList();
            ...
            )
        {
            ...
            var rs = _dbContext.SqlQuery<InventoryAdjResult>(@"exec usp_InventoryAdjSearch @fromDate, @toDate, @storeId, @status, @adjNo, @productCode, @barcode, @pageIndex, @pageSize, @count output",
                pfromDate, ptoDate, pstoreId, pstatus, padjNo, 
                pproductCode, pbarcode, ppageIndex, ppageSize, pTotalRecords).ToList();
            totalRecord = (int)pTotalRecords.Value;
        ...
        public int EditInventoryAdj(InventoryAdjResult obj)
        {
            ...
            using (TransactionScope tran = new TransactionScope())
            {
                ...
                var rs = _dbContext.SqlQuery<int>(@"exec usp_InventoryAdjEdit @StoreId, @AdjNo, @AdjDate, @Desc, @CreatedDate, @UpdatedDate, @CreatedUser, @UpdatedUser, @DebitAcctId, @ObjId, @CostAmount, @Qty",
                ...
                _dbContext.ExecuteSqlCommand(@"exec usp_InventoryAdj_Approve @adjId, @user",false,10000,
                    pAdjId, puser);
        ...
        private int insertInventoryAdjDetail(InventoryAdjLineResult line)
        {
            ...
            return _dbContext.SqlQuery<int>(@"exec usp_InventoryAdjDetailInsert @AdjId,@Seq,@BarcodeId,@Qty,@CostAmount",
            ...
            )
        {
            ...
            var rs = _dbContext.SqlQuery<InventoryTransactionResult>(@"exec [usp_InvTransactionDetails] @fromDate, @toDate, @tranNo, @productCode, @storeId, @barcode, @typeId, @pageIndex, @pageSize, @count output, @sumQty output",
                pfromDate, ptoDate, ptranNo, pproductCode, pstoreId,
                pbarcode,ptypeId, ppageIndex, ppageSize, pTotalRecords, psumQty).ToList();
            totalRecord = (int)pTotalRecords.Value;
            sumQty = (int)psumQty.Value;
            ...
            )
        {
            ...
            var rs = _dbContext.SqlQuery<InventoryTransferResult>(@"exec [usp_InvTransferSearch] @fromDate, @toDate, @tranNo, @productCode, @fromStoreId, @toStoreId, @barcode, @status, @pageIndex, @pageSize, @count output, @sumQty output",
                pfromDate, ptoDate, ptranNo, pproductCode, pfromStoreId,ptoStoreId,
                pbarcode, pstatus, ppageIndex, ppageSize, pTotalRecords, psumQty).ToList();
            totalRecord = (int)pTotalRecords.Value;
            sumQty = (int)psumQty.Value;
