...
namespace Nop.Services.Aero87
{
    public class SaleOrderService : ISaleOrderService
    {
        ...
        private readonly IRepository<POS_SaleOrder> _saleOrderRepository;

        public SaleOrderService(IDbContext dbContext, 
            IDataProvider dataProvider, 
            IRepository<POS_SaleOrder> saleOrderRepository)
        {
        ...
        public IList<SaleOrdersResult> Search(
            ...
            )
        {
            ...
            var rs = _dbContext.SqlQuery<SaleOrdersResult>(@"exec usp_SaleOrderSearch @fromDate, @toDate, @storeId, @status, @orderNo, @refNo, @customerName, @mobile, @productCode, @barcode, @desc,@carrierId,@chanelId,@sourceId,@returnReasonId,@shippingCode, @paymentType, @paymentStatus, @pageIndex, @pageSize, @count output, @sumSubTotal output, @sumDiscount output, @sumShippingFee output,@sumAeroShippingFee output, @sumAmount output,@sumCostAmount output,@sumQty output",
                pfromDate,ptoDate,pstoreId,pstatus,porderNo,prefNo,pcustomerName,pmobile,
                pproductCode, pbarcode, pdesc,pcarrierId,pchanelId,psourceId,preturnReasonId,pshippingCode,ppaymentType,ppaymentStatus, ppageIndex, ppageSize, pTotalRecords, psumSubTotal, psumDiscount, psumShippingFee,psumAeroShippingFee, psumAmount,psumCostAmount,psumQty).ToList();
        ...
        public IList<SaleOrderDetailLinesResult> GetSaleOrderDetail(int orderId)
        {
            ...
            var rs = _dbContext.SqlQuery<SaleOrderDetailLinesResult>(@"exec usp_GetPOSSaleOrderDetailBySaleOrderId @saleOrderId", porderId).ToList();
            
            return rs;
        }

        public void SaleOrderApprove(
            int orderId,
            int debitAcctId,
            int? objId,
            int? feeAcctId,
            ...
            )
        {
            ...
            _dbContext.ExecuteSqlCommand(@"exec usp_SaleOrder_Approve @orderid, @debitAcctId, @objId, @feeAcctId, @feeAmount, @desc, @user",false,10000,
                porderId, pdebitAcctId, pobjId, pfeeAcctId, pfeeAmount, pdesc, puser
                );
        }

        public void SaleOrderUnPost(
            int orderId,
            string user
            )
        {
            ...
            _dbContext.ExecuteSqlCommand(@"exec usp_SaleOrder_UnPost @orderid, @user", false, 10000,
                porderId, puser
                );
        }

        public POS_SaleOrder GetSaleOrderById(int orderId)
        {
            if (orderId == 0)
                return null;
        ...
        public void UpdateSaleOrder(UpdatedSaleOrderDTO order)
        {
            if(order == null)
            {
            ...
            _dbContext.ExecuteSqlCommand(@"exec usp_UpdateSaleOrder @Id, @IsPaid, @PaymentDate", false, 10000,
                pId, pIsPaid, pPaymentDate);
        }
    }
}
